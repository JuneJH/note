...
1. 浏览器是多进程多线程的应用
2. 其中我们主要需要关注的就是渲染主线程

## 2. 渲染主线程是如何工作的

1. 平时我们执行的代码主要就是渲染主线程
2. 因此单线程就需要考虑异步场景
3. 事件循环(消息循环)就是实现异步的方案
4. w3c规定，事件循环必须有一个微队列和任意个普通队列，微队列中的任务永远最先完成，同一类型任务必须放在同一队列，队列也可放置不同类型任务，在chrome中分别实现了微队列、交互队列、延时队列。他就考虑到用户优先策略，所以交互队列的优先级要大于延时队列
