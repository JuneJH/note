...
> 回调地狱: 在异步编程中通过传入回调进行后续操作，一旦嵌套深了，代码将会变得很难维护就会形成回调地狱 
>
> 1. Promise对象通过链式调用解决代码地狱问题
> 2. 通过Promise方案实现时序、async/awiat方案解决异步问题

## 1. Promise基本概念

1. Promise内部具有两个阶段已决/未决三种状态
   - *pending*: 初始状态
   - *fulfilled*：兑现
   - *rejected*：拒绝
2. Promise.then(): 是链式调用的基本，返回新的Promise对象,会被推入微任务队列
   1. 接受两个参数用于处理fulfiled/rejected
...
> async和await 是promise 的语法糖

1. 如果被async标记的函数返回的是一个promise，那么该函数返回和此promise相同的状态

   ```js
   async function m(){
     return new Promise(); // 此函数的promise状态为返回promise的状态
   }

2. 如果函数执行过程中报错了，该函数返回的promise为reject
...
1. 使用awiat 执行一个函数就会立即被执行，后续代码需要该promise完成后放入微队列等待执行

2. 调用then方法不是函数

   ```js
   Promise.resolve().then(1).then(2); // 相当于没有传递任何处理函数，即无效处理
   ```
